...
export const gradutationYearValidation = (
  batchName,
  admissionYear,
  graduationYear,
  currentYear
) => {
  console.log(currentYear, admissionYear);
  if (admissionYear < 1998 || admissionYear > currentYear) {
    triggerAlert({ icon: 'error', title: 'Enter correct Admission Year' });
    return 0;
  }
  if (batchName === 'PhD') {
    return 1;
